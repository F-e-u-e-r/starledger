...
export function deriveRepo(repo: CanonicalRepo, now: Date): DerivedRepo {
...
export function deriveAll(repos: readonly CanonicalRepo[], now: Date): DerivedRepo[] {
  return repos.map((repo) => deriveRepo(repo, now));
}
